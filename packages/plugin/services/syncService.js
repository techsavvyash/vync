...
class SyncService {
    ...
    async getFilesToSync(vaultPath) {
        const files = [];
        const relevantExtensions = ['.md', '.txt', '.pdf', '.png', '.jpg', '.jpeg'];
        try {
            const entries = await fs_1.promises.readdir(vaultPath, { withFileTypes: true });
            for (const entry of entries) {
                const fullPath = path_1.default.join(vaultPath, entry.name);
    ...
    async downloadFiles(remoteFiles, vaultPath) {
        ...
        for (const remoteFile of remoteFiles) {
            try {
                ...
                const localPath = path_1.default.join(vaultPath, remoteFile.filePath);
                ...
                if (!exists) {
                    ...
                    if (response.ok) {
                        ...
                        if (result.success && result.data) {
                            // Write file to disk
                            await fs_1.promises.writeFile(localPath, result.data.fileData, 'base64');
                            results.push(result);
    ...
    async syncOnChange(vaultPath, changedFiles) {
